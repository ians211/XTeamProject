...
/**
 * This class creates the GUI and displays the quiz to the user.
 */
public class Main extends Application {

    private QuestionDataBase questionBank = new QuestionDataBase(); // holds all of the questions
     ...
     *
     * @param primaryStage the stage for the GUI to be displayed in
     */
    private void addQuestion(Stage primaryStage) {
        ...
        textFieldVBox.getChildren().addAll(enterQuestionHBox, enterAnswerHBox, enterOption1HBox, enterOption2HBox,
                enterOption3HBox, enterOption4HBox, topicHBox, chooseImageHBox, enterFilepathHBox);
        ...
        submit.setOnAction(e -> {
            ...
            if (topicBox.getValue() == null) {
                validInputs = false;
                // could not figure out how to change prompt text color of combobox so changed
                // prompt text instead
            ...
            if (validInputs) {
                ...
                Question currQ = new Question("unused", enterQuestionTextField.getText(), topicBox.getValue(), image,
                        choices);
    ...
    /**
     * This method sets up and displays the GUI for the screen where you can select
     * quiz topics and number of questions desired.
     *
     * @param primaryStage the stage for the GUI to be displayed in
     */
    private void selectQuizTopics(Stage primaryStage) {
        ...
        message.setFont(new Font("Palatino Linotype", 50));
        message.setPadding(new Insets(10));
        root.setTop(message);

        // get and sort the topics in alphabetical order
        ...
        // textfield for number of questions
        Label numQuestionsLabel = new Label("Number of questions in quiz:");
        numQuestionsLabel.setFont(new Font("Palatino Linotype", 20));
        ...
        // creating buttons at bottom
        Button cancel = new Button("Cancel");
        Button next = new Button("Start Quiz");
        cancel.setFont(new Font("Palatino Linotype", 20));
        next.setFont(new Font("Palatino Linotype", 20));
        ...
        next.setOnAction(e -> {
            ...
            if (num == -1) {
                ...
                numQuestions.setPromptText("You must input a natural number"); // check for invalid input
    ...
    /**
     * This method returns the number of questions in the quiz.
     *
     * @param numQuestions
    ...
    /**
     * This method initializes all the questions of the topics selected by the user.
     *
     * @param topics
     * @param checkMarks
     * @return ArrayList of all the questions for those topics selected
     */
    ...
    /**
     * This method sets up and displays the GUI for the quiz screen.
     *
     * @param primaryStage the stage for the GUI to be displayed in
     * @param allQuestions the array of questions in the quiz
    ...
    private void quizScreen(Stage primaryStage, ArrayList<Question> allQuestions, int numQuestions) {
        Quiz quiz = new Quiz(allQuestions, numQuestions); // new quiz created
        ArrayList<Question> questionList = quiz.getQuestionList(numQuestions); // question list
        ...
        Collections.shuffle(qChoices); // randomize the choice order
        ...
        ImageView imageView = questionList.get(0).getImage();
        ...
        // bottom bar ofbuttons
        enterButton.setOnAction(enterEvent);
        buttonBar.getButtons().addAll(enterButton, nextButton);
    ...
    /**
     * This method sets up and displays the GUI for the results screen.
     *
     * @param primaryStage the stage for the GUI to be displayed in
     * @param quiz         contains all of the questions
     **/
    private void results(Stage primaryStage, Quiz quiz) {
        ...
        results.setFont(new Font("Palatino Linotype", 80));
        root.setTop(results);
        ...
        finalScore.setFont(new Font("Palatino Linotype", 30));
        answered.setFont(new Font("Palatino Linotype", 30));
        answeredCorrectly.setFont(new Font("Palatino Linotype", 30));
        totalQuestions.setFont(new Font("Palatino Linotype", 30));
        ...
        home.setFont(new Font("Palatino Linotype", 20));
        newQuiz.setFont(new Font("Palatino Linotype", 20));
        exit.setFont(new Font("Palatino Linotype", 20));
        exitS.setFont(new Font("Palatino Linotype", 20));
        ...
        exit.setOnAction(e -> {
            ((Stage) primaryStage.getScene().getWindow()).close(); // close window
        });

        // action for exiting and saving questions
        exitS.setOnAction(e -> {
            ArrayList<String> topics = questionBank.getTopics();
            JSONArray questionBankJSON = new JSONArray();
    ...
    public void readFile(String jsonPath) {
        ...
        try {
            ...
            for (int j = 0; j < ja.size(); j++) {
                ...
                JSONArray jsonChoices = (JSONArray) question.get("choiceArray");
                // cast to JSONArray, get the choices
    ...
    /**
     * This method sets the question in the quiz with its according attributes.
     *
     * @param question
     * @param label
     * @param vbox
    ...
    public void setQuestion(Question question, Label label, VBox vbox, ToggleGroup group, Quiz quiz,
                            ImageView imageView) {
        // label of question
        label.setText("Question #" + (quiz.getQuestionNum() + 1) + " out of " + quiz.getNumQuestions() + ": "
                + question.getQuestion());

        // choices array that is shuffled in random order
        ...
        // set image
        imageView.setImage(question.getImage().getImage());
        imageView.setFitHeight(300);
        imageView.setFitWidth(400);
    }
